...
export const RandomResultsView: React.FC = () => { 
  ...
  useEffect(() => {
    const fetchWinner = async () => {
      ...
      try {
        const response = await fetch(`/api/events/winner?eventId=${eventData.id}`);
  ...
  return (
    <div className="p-4 border rounded-lg bg-white shadow">
      <h2 className="text-xl font-semibold mb-4 text-center">Random Event Results</h2>
      
      {isLoading && (
        <div className="text-center text-gray-500">Loading winner information...</div>
      )}
